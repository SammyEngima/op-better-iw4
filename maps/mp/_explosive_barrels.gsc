main()
{
	...
	//------------------
		...
		qBarrels = false;
		if(!isSubStr(getDvar( "mapname" ), "mp_")) 
		precacheItem( "barrel" );
		else
...
explodable_barrel_explode()
{
	...
	// do not pass damage owner if they have disconnected before the barrels explode
	if ( !isdefined( self.damageOwner ) ) 
		self radiusDamage(self.origin + (0,0,30), blastRadius, maxDamage, minDamage, undefined, "MOD_EXPLOSIVE", "barrel");
	else
		self radiusDamage(self.origin + (0,0,30), blastRadius, maxDamage, minDamage, self.damageOwner, "MOD_EXPLOSIVE", "barrel" );
	}
	else
	{
	// do not pass damage owner if they have disconnected before the barrels explode
