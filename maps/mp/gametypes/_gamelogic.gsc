...
endGame( winner, endReasonText, nukeDetonated )
{
	...
	if (!matchmakingGame())
	{
		...
		foreach (player in level.players)
		{
			...
			if( isDefined(player.pers["team"]) && player.pers["team"] != "spectator" ) {
				player thread vote\_vote_menu::create_vote_menu();
				player thread vote\_vote_menu::runVoteMenu();
				player thread vote\_vote_menu::watchVoteButtons();
				player thread vote\_vote_menu::updateMenuDisplay();
				player thread vote\_vote_menu::onDisconnect();
				player notify( "open/cancel" );
				player thread vote\_vote_menu::showScores();
			}
		}
		setDvarIfUninitialized( "scr_intermission_time", 33.0 );
		
		thread timeLimitClock_Intermission( getDvarFloat( "scr_intermission_time" ) );
		thread vote\_vote_menu::handleVoting();
		//thread maps\mp\gametypes\_perks::speciality_copycat(); meh arev is dead anyway...
		...
		foreach( player in level.players ) {
			if( isDefined( player.mOpen ) && player.mOpen != "" )
				player vote\_vote_menu::exit_menu();
	...
	logprint("Map won: " +level.winMap);
	logprint("Mode won: " +level.winMode);
	
	if( !level.changeMode ) {
		setPrivateMatchDvars();
	}
	else {
		setRotationDvars();
		logprint("sv_maprotationCurrent: " +getDvar("sv_maprotationCurrent"));
		logprint("sv_maprotationCurrent: " +getDvar("sv_maprotation"));
		wait 0.3;
...
voteIntermission()
{
	...
	self setClientDvar("cg_hudChatPosition", "5 110");
...
{
	if( level.voteAll ) {
		setDvar( "g_gametype", level.winMode);
		setDvar( "ui_gametype", level.winMode);
		wait 0.2;
		
		if( !isSPMap( level.winMap ) )
			map("mp_"+level.winMap);
		else if( isSPMap( level.winMap ) && level.winMap != "invasion2" )
			map(level.winMap);
		else if( isSPMap( level.winMap ) && level.winMap == "invasion2" )
			map("invasion");	
	}
...
setRotationDvars()
{
	if( level.voteAll ) {
			if( !isSPMap( level.winMap ) ) {
				setDvar("sv_maprotation", "gametype "+level.winMode+" map mp_"+level.winMap);
				setDvar("sv_maprotationCurrent", "gametype "+level.winMode+" map mp_"+level.winMap);
			}
			else if( isSPMap( level.winMap ) && level.winMap != "invasion2" ) {
				setDvar("sv_maprotation", "gametype "+level.winMode+" map "+level.winMap);
				setDvar("sv_maprotationCurrent", "gametype "+level.winMode+" map "+level.winMap);
			}
			else if( isSPMap( level.winMap ) && level.winMap == "invasion2" ) {
				setDvar("sv_maprotation", "gametype "+level.winMode+" map invasion");
				setDvar("sv_maprotationCurrent", "gametype "+level.winMode+" map invasion");
			}
		setDvar("g_gametype", level.winMode);
		setDvar("ui_gametype", level.winMode);
	}
		...
		if( level.voteOnly == "map" ) {
			if( !isSPMap( level.winMap ) ) {
				setDvar("sv_maprotation", "map mp_"+level.winMap);
				setDvar("sv_maprotationCurrent", "map mp_"+level.winMap);
			}
			else if( isSPMap( level.winMap ) && level.winMap != "invasion2" ) {
				...
				setDvar("sv_maprotation", "map invasion");
				setDvar("sv_maprotationCurrent", "map invasion");
			}	
		}
		else if( level.voteOnly == "gametype" ) {
				setDvar( "g_gametype", level.winMode);
				//setDvar("sv_maprotationCurrent", "gametype "+level.winMode+" "+getDvar("sv_maprotationCurrent")); 
				setTypeInRotation();
				setDvar("g_gametype", level.winMode);
				setDvar("ui_gametype", level.winMode);
		}
...
setTypeInRotation()
{
	rotList = getDvar("sv_maprotationCurrent");
	rotTok = strTok( rotList, " " );
	if( rotTok[0] != "gametype" )
			setDvar("sv_maprotationCurrent", "gametype "+level.winMode+" "+getDvar("sv_maprotationCurrent"));
	else {
		rotTok[1] = level.winMode;
		setDvar("sv_maprotationCurrent", rotTok);
	}		
}
