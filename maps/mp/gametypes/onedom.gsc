...
/*
	Domination
	Objective: 	Capture the flag and hold it
	Map ends:	When a team captures all the flags until scorelimit is reached
...
main()
{
	...
	registerTimeLimitDvar( "onedom", 30, 0, 1440 );
	registerScoreLimitDvar( "onedom", 100, 0, 1000 );
	registerRoundLimitDvar( "onedom", 1, 0, 10 );
	registerWinLimitDvar( "onedom", 1, 0, 10 );
	registerNumLivesDvar( "onedom", 0, 0, 10 );
	registerHalfTimeDvar( "onedom", 0, 0, 1 );
...
onStartGameType()
{	
	setObjectiveText( "allies", "Hold and Capture the flag" );
	setObjectiveText( "axis", "Hold and Capture the flag" );

	if ( level.splitscreen )
	{
		setObjectiveScoreText( "allies", "Hold and Capture the flag" );
		setObjectiveScoreText( "axis", "Hold and Capture the flag" );
	}
	else
	{
		setObjectiveScoreText( "allies", &"Hold and Capture the flag" );
	...
	setObjectiveHintText( "allies", "Hold and Capture the flag" );
	setObjectiveHintText( "axis", "Hold and Capture the flag" );
...
domFlags()
{
	...
	primaryFlags = getEntArray( "dom_one_flag_trigger", "targetname" );
